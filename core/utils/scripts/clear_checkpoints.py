...
def find_weight_folders():
    ...
    for root, dirs, files in os.walk(os.getcwd(), followlinks=True):
        for dir in dirs:
            if dir.endswith("weights"):
                dirnames.append(os.path.join(root, dir))
    return dirnames
...
if __name__ == "__main__":
    if not query_yes_no(
        "Deleting all non-latest checkpoints from {}, CONTINUE?".format(os.getcwd())
    ):
        print("Aborting...")
        exit(0)
    ...
    if not query_yes_no(
        "Will delete checkpoints from {} directories, CONTINUE?".format(
            len(checkpt_dirs)
        )
    ):
        print("Aborting...")
