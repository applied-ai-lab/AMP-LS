...
from core.utils.general_utils import get_dim_inds
from core.utils.pytorch_utils import ten2ar, ar2ten
...
@metric
def mean_angle_btw_vectors(estimates, targets):
    if isinstance(estimates, torch.Tensor):
        estimates = ten2ar(estimates)
    if isinstance(targets, torch.Tensor):
        targets = ten2ar(targets)

    dot_product = np.array([np.dot(estimates[0, i], targets[0, i]) for i in range(3)])
    cos_a = dot_product / (
        np.linalg.norm(estimates, axis=-1)[0] * np.linalg.norm(targets, axis=-1)[0]
    )
    eps = 1e-8
    cos_a = np.clip(cos_a, -1 + eps, 1 - eps)
    angle_dist = np.arccos(cos_a) / np.pi * 180.0

    return np.mean(angle_dist)
