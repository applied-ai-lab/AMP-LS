...
class NoneGradHook:
    ...
    def run(self, inputs, output, losses, epoch):
        none_list = [
            n
            for n, p in filter(
                lambda x: x[1] is None, self.trainer.model.named_parameters()
            )
        ]
        if none_list:
            print(none_list)
...
def check_shape(t, target_shape):
    if not list(t.shape) == target_shape:
        raise ValueError(
            f"Temsor should have shape {target_shape} but has shape {list(t.shape)}!"
        )
...
def mask_out(tensor, start_ind, end_ind, value, dim=1):
    ...
    inds = torch.arange(time, device=tensor.device, dtype=start_ind.dtype).expand(
        batch_size, -1
    )
...
def get_padding(seq, replace_dim, size, val=0.0):
    ...
    if isinstance(seq, torch.Tensor):
        return val * torch.ones(
            seq_shape[:replace_dim] + (size,) + seq_shape[replace_dim + 1 :],
            device=seq.device,
        )
    else:
        return val * np.ones(
...
def pad_seq(seq, pre=0, post=0, length=None, val=0.0, dim=1):
    ...
    if seq.shape[dim] < length:
        return ndim.cat(
            (seq, get_padding(seq, dim, length - seq.shape[dim], val)), dim=dim
        )
    else:
...
def stack_with_separator(tensors, dim, sep_width=2):
    ...
    if len(tensors) > 1:
        for tensor in tensors[1:]:
            ...
            )  # all stacked tensors must have same shape!
...
class ExponentialDecayUpdater(Updater):
    ...
    def step(self):
        if (
            self.it % self.update_freq == 0
            and self.parameter.data[0] * self.decay > self.min_limit
        ):
...
def get_constant_parameter(init_log_value, learnable):
    return torch.nn.Parameter(
        torch.full((1,), init_log_value)[0], requires_grad=learnable
    )
...
def sample_idx_batch_in_range(start, end, batch_size=None, device=None):
    ...
    idxs = (
        torch.rand((bs,), device=device) * (end.float() - start.float() - 1)
        + start.float()
    )
    return idxs.long()
...
if __name__ == "__main__":
    ...
    def test_stack_pad(generalized_tensors):
        generalized_tensors = [
            pad_seq(t, pre=1, length=10) for t in generalized_tensors
        ]
