...
class BaseTrainer:
    def override_defaults(self, policyparams):
        for name, value in policyparams.items():
            ...
            if value == getattr(self._hp, name):
                raise ValueError(
                    "attribute is {} is identical to default value!!".format(name)
                )
    ...
    def nan_grads_hook(self, inputs, output, losses, epoch):
        non_none = list(
            filter(lambda x: x[1].grad is not None, self.model.named_parameters())
        )
        if self.check_nan_grads(non_none):
            self.dump_debug_data(inputs, output, losses, epoch)
            nan_parameters = [
                n for n, v in filter(lambda x: torch.isnan(x[-1].grad).any(), non_none)
            ]
            non_nan_parameters = [
                n
                for n, v in filter(
                    lambda x: not torch.isnan(x[-1].grad).any(), non_none
                )
            ]
            ...
            def get_nan_module(layer):
                return np.unique(
                    map(lambda n: ".".join(n.split(".")[:layer]), nan_parameters)
                )
    ...
    def print_nan_grads(self):
        ...
        for name, param in self.model.named_parameters(recurse=True):
            print(
                "{}:\t\t{}".format(
                    name, bool(torch.isnan(param.grad).any().data.cpu().numpy())
                )
            )
