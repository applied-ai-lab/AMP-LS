...
from core.utils.eval_utils import ssim, psnr, mse, mean_angle_btw_vectors
from core.utils.general_utils import batchwise_index, AttrDict, timed, dict_concat
from core.utils.vis_utils import make_gif_strip
from core.utils.pytorch_utils import ten2ar
from core.utils.transform_utils import euler2mat, quat2mat
...
class TopOfNEvaluator:
    ...
    def __init__(self, hp, logdir, top_of_n, top_comp_metric, logger=None):
        self._hp = hp
        self._logdir = logdir + "/eval"
        if self._hp.logging_target == "wandb":
            self._logger = WandBEvalLogger(logdir, logger)
        elif self._hp.logging_target == "tb":
            self._logger = TBEvalLogger(logdir, logger)
        else:
            self._logger = FileEvalLogger(self._logdir)
...
class MultiImageEvaluator(ImageEvaluator):
    def _mse(self, inputs, model_outputs, batch_idx):
        return mse(
            model_outputs.output_imgs[batch_idx],
            inputs.images[
                batch_idx,
                -model_outputs.output_imgs.shape[1] :,
                -model_outputs.output_imgs.shape[2] :,
            ],
        )
    ...
    def _store_aux_outputs(self, inputs, model_outputs, batch_idx):
        return AttrDict(
            gt=inputs.images[batch_idx],
            estimate=model_outputs.output_imgs[
                batch_idx, :, -model_outputs.output_imgs.shape[2] :
            ],
        )
...
class RotationEvaluator(Evaluator):
    """Implements Evaluator for data that evaluates Rotation."""

    @property
    def metrics(self):
        return AttrDict(rot=self._rot)

    def _rot(self, inputs, model_outputs, batch_idx):
        rot_dim = 6
        output_q = model_outputs.decoded_output[batch_idx, :, -rot_dim:]
        input_q = inputs.states[batch_idx, :, -rot_dim:]

        return mean_angle_btw_vectors(
            self._get_rotated(input_q), self._get_rotated(output_q)
        )

    def _get_rotated(self, x):
...
class WandBEvalLogger(EvalLogger):
    """Logs to WandB."""
    ...
    def _log_metric(self, name, vals, best_vals):
        self._logger.log_scalar_dict(
            {"val/{}/top100_mean".format(name): best_vals.mean()},
            prefix="",
            step=self.log_step,
        )
        self._logger.log_scalar_dict(
            {"val/{}/top100_std".format(name): best_vals.std()},
            prefix="",
            step=self.log_step,
        )
        if vals is not None:
            self._logger.log_scalar_dict(
                {"val/{}/all100_mean".format(name): vals.mean()},
                prefix="",
                step=self.log_step,
            )
            self._logger.log_scalar_dict(
                {"val/{}/all100_std".format(name): vals.std()},
                prefix="",
                step=self.log_step,
            )
    ...
    def _log_gif(self, gif):
        self._logger.log_videos(self, gif, step=self.log_step)
