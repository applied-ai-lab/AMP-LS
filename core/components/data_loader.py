...
from core.utils.pytorch_utils import RepeatedDataLoader
from core.utils.video_utils import resize_video
...
class Dataset(data.Dataset):
    def __init__(self, data_dir, data_conf, phase, shuffle=True, dataset_size=-1):
        ...
        self.shuffle = shuffle or phase == "train"
        # self.n_worker = 20 if shuffle else 1  # was 4 before
        self.n_worker = 20
    ...
    def _get_filenames(self):
        ...
        for task_name in self.spec.task_names:
            ...
            for root, dirs, files in os.walk(os.path.join(self.data_dir, task_name)):
                for file in files:
                    if file.endswith(".h5"):
                        task_filenames.append(os.path.join(root, file))
...
class ImageDataset(Dataset):
    """Generic image dataset. Assumes that hdf5 file has images/pad_mask."""

    DATA_KEYS = ["images", "pad_mask"]
    ...
    def __getitem__(self, index):
        ...
        data.images = data.images[start : start + 1]
...
class GlobalSplitImageDataset(ImageDataset, GlobalSplitDataset):
    pass
...
class FixedLenDummySubskillSequenceDataset(GeneratedVideoDataset):
    ...
    def __init__(self, *args, **kwargs):
        ...
        possible_seg_lengths = np.arange(
            self.spec.min_segment_len, self.spec.max_segment_len + 1
        )
        self.possible_splits = np.stack(
            np.meshgrid(*([possible_seg_lengths] * self.spec.num_subskills_per_seq))
        ).reshape(self.spec.num_subskills_per_seq, -1)
        self.possible_splits = self.possible_splits[
            :, self.possible_splits.sum(0) == self.spec.max_seq_len
        ]
        if "static_split" in self.spec and self.spec.static_split:
            self.possible_splits = np.asarray(
                [
                    [len(s)]
                    for s in np.array_split(
                        range(self.spec.max_seq_len), self.spec.num_subskills_per_seq
                    )
                ]
            )
    ...
    def get_sample(self):
        ...
        temp_split = self.possible_splits[
            :, np.random.randint(self.possible_splits.shape[1], size=1)
        ]
        subskills = np.random.choice(
            np.arange(0, self.spec.num_subskills), temp_split.size, replace=False
        )
        traj = []
        ...
        data_dict.images = np.tile(
            traj[..., None, None, None], (1, 3, self.img_sz, self.img_sz)
        ).astype(np.float32)
        data_dict.states = np.tile(traj[..., None], (1, self.spec["state_dim"])).astype(
            np.float32
        )
        data_dict.actions = np.tile(
            traj[:-1, None], (1, self.spec["n_actions"])
        ).astype(np.float32)
