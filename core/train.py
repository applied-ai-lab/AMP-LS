...
from core.utils.general_utils import RecursiveAverageMeter, map_dict
...
from core.components.trainer_base import BaseTrainer
from core.components.params import get_args
from core.utils.wandb import WandBLogger

WANDB_PROJECT_NAME = "lsmp"
...
class ModelTrainer(BaseTrainer):
        ...
        for self.batch_idx, sample_batched in enumerate(self.train_loader):
            ...
            if self.log_outputs_now:
                ...
                togo_train_time = (
                    batch_time.avg
                    * (
                        (self._hp.num_epochs - epoch - 1) * epoch_len
                        + len(self.train_loader)
                        - self.batch_idx
                    )
                    / 3600.0
    ...
    @staticmethod
    def override_conf_cmd_line(config, override_args):
        def recursive_param_override(conf, key, val):
            if not isinstance(key, list):
                conf[key] = val
            elif len(key) == 1:
                conf[key[0]] = val
            else:
                recursive_param_override(conf[key[0]], key[1:], val)
        ...
        for i in range(0, len(override_args), 2):
            ...
            if "." in key:
                key = key.split(".")
            value = eval(value)
            recursive_param_override(config, key, value)
        return config
