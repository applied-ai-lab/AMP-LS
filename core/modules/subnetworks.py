...
from core.modules.variational_inference import Gaussian
from core.modules.variational_inference import SequentialGaussian_SharedPQ
from core.modules.variational_inference import UnitGaussian
...
from core.utils.general_utils import broadcast_final, AttrDict
...
class ConvEncoder(nn.Module):
    def __init__(self, hp):
        ...
        # self.net.add_module('head', conv_cls(hp.ngf * 2 ** (n - 3), hp.nz_enc, kernel))
        self.net.add_module(
            "head",
            block(
                in_dim=hp.ngf * 2 ** (n - 3),
...
class Conv3dEncoder(nn.Module):
    def __init__(self, hp):
        ...
        self.net.add_module("input-pool", nn.MaxPool3d(2, stride=2))
        for i in range(n - 3):
            filters_in = hp.ngf * 2**i
            self.net.add_module(
                "pyramid-{}".format(i),
            ...
            self.net.add_module("pyramid-pool-{}".format(i), nn.MaxPool3d(2, stride=2))

        self.net.add_module(
            "head",
            nn.ConvTranspose3d(
                hp.ngf * 2 ** (n - 3), hp.nz_enc, kernel_size=2, stride=2
            ),
        )
        # self.net.add_module('head', ConvBlock(in_dim=hp.ngf * 2 ** (n - 3), out_dim=hp.nz_enc, normalize=hp.builder.normalize,
        #                                       builder=hp.builder))
