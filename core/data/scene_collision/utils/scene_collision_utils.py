...
class ProcessKillingExecutor:
    ...
    @staticmethod
    def _process_run(queue: Queue, func: Callable[[Any], Any] = None, *args, **kwargs):
        """
...
def process_mesh(in_path, out_path, scale, grasps, return_stps=True):
    ...
    mesh.apply_transform(trimesh.transformations.translation_matrix(-mesh_offset))
    m_scale = "{}_{}.obj".format(os.path.splitext(os.path.basename(out_path))[0], scale)
